...
export function buildPlan({polygons,communities,instrumentCount,project,unproject,config={}}){
  ...
  const fixedPoints=points.filter(p=>p.fixed===true),variablePoints=points.filter(p=>p.fixed!==true);
  const options=combinations(variablePoints,instrumentCount-fixedPoints.length).map(selected=>[...fixedPoints,...selected]);
  ...
  return {origin,polygons:areas,communities:points,scenarios,instrumentCount,fixedPointCount:fixedPoints.length,warnings:[...(fixedPoints.length?[`${fixedPoints.length} ponto(s) fixo(s) incluído(s) nos três cenários.`]:[]),...(directed.length<areas.length?[`${areas.length-directed.length} área(s) sem direção informada; o direcionamento não é avaliado nessas áreas.`]:[]),...(!directed.length?['Cenário 2 usa proximidade porque nenhuma direção foi informada.']:[]),...(scenarios.some(s=>s.coincidentWith.length)?['Os critérios podem selecionar os mesmos pontos. Cenários coincidentes são identificados.']:[])]};
}
