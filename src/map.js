...
export function createCampaignMap(elementId, configuration={}) {
 ...
 const palette={area:'#fa1232',circle:'#06d9e8',selected:'#fa1232',available:'#254354',direction:'#ffca28',...(configuration.visual?.mapColors || {})};
 ...
 function update(next){data={...data,...next};overlays.clearLayers();const selected=new Set(data.selectedIds || []);
  ...
  for(const community of data.communities || []){if(community.enabled===false)continue;const chosen=selected.has(community.id),lat=Number(community.lat),lon=Number(community.lon);if(!Number.isFinite(lat)||!Number.isFinite(lon))continue;
   ...
   L.circleMarker([lat,lon],{radius:chosen?8:5,color:chosen?'#fff':palette.available,weight:chosen?2:1.5,fillColor:chosen?palette.selected:'#fff',fillOpacity:1}).bindTooltip(`${escapeHtml(community.name)}${chosen?' · selecionado':''}`,{permanent:true,direction:'top',offset:[0,-5],className:'community-tooltip'}).addTo(overlays);
  }
 }
 ...
 async function exportCanvas({width=1400,height=900,includeSatellite=true}={}){
  ...
  const selected=new Set(data.selectedIds||[]);for(const community of data.communities||[]){if(community.enabled===false)continue;const p=xy(community.lon,community.lat),chosen=selected.has(community.id);if(chosen&&data.origin){const origin=xy(data.origin.lon,data.origin.lat);line([origin,p],satellite?'#ffffffcc':'#647983',2,true);if(Number.isFinite(community.distance)){const text=`${Math.round(community.distance).toLocaleString('pt-BR')} m`;label(text,(origin[0]+p[0])/2,(origin[1]+p[1])/2);}}ctx.beginPath();ctx.arc(...p,chosen?9:6,0,Math.PI*2);ctx.fillStyle=chosen?palette.selected:'#fff';ctx.fill();ctx.lineWidth=2;ctx.strokeStyle=chosen?'#fff':palette.available;ctx.stroke();ctx.font=`600 ${Math.max(13,width/90)}px Arial`;const text=community.name,textWidth=ctx.measureText(text).width;label(text,Math.max(8,Math.min(width-textWidth-8,p[0]-textWidth/2)),p[1]-16);}
